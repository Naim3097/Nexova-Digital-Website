    ...
    <div>
      ...
      <section className="max-w-[1400px] mx-auto px-6 lg:px-12 py-20 lg:py-28">
        <div className="grid lg:grid-cols-2 gap-12 items-end">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-brand-lightRed font-semibold mb-6">Journal</p>
      ...
      {featured && (
        <section className="border-t border-brand-darkRed/10 py-16 px-6 lg:px-12">
          <div className="max-w-[1400px] mx-auto grid lg:grid-cols-5 gap-10 items-center">
            <div className="lg:col-span-2 aspect-[4/3] bg-gradient-to-br from-brand-lightRed/10 to-brand-darkRed/5 flex items-end p-8">
              <p className="text-xs uppercase tracking-[0.3em] text-brand-lightRed font-semibold">Featured</p>
            </div>
            <div className="lg:col-span-3 lg:pl-8">
              <span className="text-xs uppercase tracking-[0.2em] text-brand-lightRed font-semibold">{featured.category}</span>
              <h2 className="text-3xl md:text-4xl font-heading font-bold mt-2 mb-4 leading-tight">{featured.title}</h2>
              <p className="text-brand-darkRed/60 mb-6 text-lg leading-relaxed max-w-lg">{featured.excerpt}</p>
              <div className="flex items-center gap-6 text-xs text-brand-darkRed/40 mb-8">
                <span className="flex items-center gap-1"><Clock size={12} />{featured.date}</span>
                <span>{featured.readTime}</span>
              </div>
              <Link to="#" className="group inline-flex items-center gap-2 font-semibold text-brand-darkRed border-b-2 border-brand-darkRed pb-1 hover:text-brand-lightRed hover:border-brand-lightRed transition">
      ...
      {/* Filter & Search */}
      <section className="border-t border-brand-darkRed/10 py-8 px-6 lg:px-12">
        <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              ...
              <button
                ...
                className={`px-4 py-2 text-xs uppercase tracking-wider font-semibold transition ${
                  activeCategory === cat
                    ...
                    : 'text-brand-darkRed/50 hover:text-brand-darkRed'
                }`}
              >
                {cat}
              </button>
          ...
          <div className="relative">
            <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-brand-darkRed/30" />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Search articles..."
              className="pl-9 pr-4 py-2 border border-brand-darkRed/10 text-sm bg-transparent focus:outline-none focus:border-brand-lightRed w-64"
            />
          </div>
        </div>
      </section>
      ...
      <section className="max-w-[1400px] mx-auto px-6 lg:px-12 pb-24">
        <div className="divide-y divide-brand-darkRed/10">
          {filtered.map((article, i) => (
            <article key={article.id} className="group py-8 grid lg:grid-cols-12 gap-6 items-start">
              <div className="lg:col-span-1 text-3xl font-heading font-bold text-brand-darkRed/10">
                {String(i + 1).padStart(2, '0')}
              </div>
              <div className="lg:col-span-7">
                <span className="text-xs uppercase tracking-[0.2em] text-brand-lightRed font-semibold">{article.category}</span>
                <h3 className="text-xl md:text-2xl font-heading font-bold mt-1 mb-2 group-hover:text-brand-lightRed transition leading-snug">
                  {article.title}
                </h3>
                <p className="text-sm text-brand-darkRed/60 max-w-lg">{article.excerpt}</p>
              </div>
              <div className="lg:col-span-2 text-xs text-brand-darkRed/40 pt-2">
              ...
              <div className="lg:col-span-2 flex items-start justify-end pt-2">
                <Link to="#" className="inline-flex items-center gap-1 text-sm font-semibold text-brand-darkRed opacity-0 group-hover:opacity-100 transition">
        ...
        {filtered.length === 0 && (
          <p className="text-center text-brand-darkRed/40 py-16 text-lg">No articles found.</p>
        )}
      </section>
      ...
      <section className="bg-white py-24 px-6 lg:px-12">
        <div className="max-w-[1400px] mx-auto">
          <div className="grid lg:grid-cols-3 gap-16 items-start">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-brand-lightRed font-semibold mb-3">Explore</p>
              <h2 className="text-4xl font-heading font-bold leading-tight">Topics We Cover</h2>
            </div>
            <div className="lg:col-span-2 flex flex-wrap gap-3">
              {topics.map((topic, i) => (
                <span key={i} className="px-5 py-3 border border-brand-darkRed/10 text-sm hover:border-brand-lightRed hover:text-brand-lightRed transition cursor-pointer flex items-center gap-2">
                  <Tag size={12} /> {topic}
                </span>
              ))}
            </div>
      ...
      {/* Newsletter */}
      <section className="border-t border-brand-darkRed/10 py-24 px-6 lg:px-12">
        <div className="max-w-[700px] mx-auto text-center">
          <p className="text-xs uppercase tracking-[0.3em] text-brand-lightRed font-semibold mb-4">Newsletter</p>
          <h2 className="text-4xl font-heading font-bold mb-4">Stay in the Loop</h2>
          <p className="text-brand-darkRed/60 mb-8">Get our latest insights on branding and social media — delivered to your inbox. No spam, just value.</p>
          <form className="flex flex-col sm:flex-row gap-0" onSubmit={(e) => e.preventDefault()}>
            <input
              type="email"
              placeholder="your@email.com"
              className="flex-grow px-5 py-4 border border-brand-darkRed/10 border-r-0 sm:border-r-0 bg-transparent focus:outline-none focus:border-brand-lightRed text-sm"
            />
            <button className="bg-brand-darkRed text-white px-8 py-4 text-sm font-semibold uppercase tracking-wider hover:bg-brand-lightRed transition shrink-0">
              Subscribe
            </button>
          </form>
          <p className="text-xs text-brand-darkRed/30 mt-4">We respect your privacy. Unsubscribe anytime.</p>
        </div>
      </section>
    </div>
  );
