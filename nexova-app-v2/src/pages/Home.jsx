    ...
    <div>
      ...
      <section className="min-h-[90vh] flex flex-col justify-center px-6 lg:px-12 max-w-[1400px] mx-auto py-20">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-brand-lightRed font-semibold mb-6">Social Media Design & Brand Kits</p>
            ...
            <p className="text-lg text-brand-darkRed/70 max-w-md leading-relaxed mb-10">
            ...
            <div className="flex flex-wrap gap-4">
              <Link to="/contact" className="group inline-flex items-center gap-2 bg-brand-darkRed text-white px-8 py-4 font-semibold hover:bg-brand-darkRed/90 transition">
              ...
              <Link to="/product" className="inline-flex items-center gap-2 border-2 border-brand-darkRed/20 text-brand-darkRed px-8 py-4 font-semibold hover:border-brand-darkRed transition">
                View Packages
              </Link>
            </div>
          </div>
          <div className="relative">
            <div className="aspect-[3/4] bg-brand-darkRed/5 relative overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-t from-brand-darkRed/30 to-transparent flex items-end p-8">
                <span className="text-white/60 font-heading italic text-lg">[ Creative Team Photography ]</span>
              </div>
            </div>
            {/* Floating stat card */}
            <div className="absolute -bottom-6 -left-6 bg-white p-6 shadow-xl border-l-4 border-brand-lightRed">
              <p className="text-4xl font-heading font-bold">150+</p>
              <p className="text-sm text-brand-darkRed/60">Brands Transformed</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="border-y border-brand-darkRed/10 py-8 overflow-hidden">
        <div className="flex items-center gap-16 animate-marquee whitespace-nowrap px-6">
          {["Brew & Bean", "FitVibe", "Luna Skincare", "UrbanNest", "GreenLeaf", "Apex", "Brew & Bean", "FitVibe", "Luna Skincare", "UrbanNest"].map((brand, i) => (
            <span key={i} className="font-heading italic text-2xl text-brand-darkRed/20 shrink-0">{brand}</span>
          ))}
        </div>
      </section>

      {/* Stats — Horizontal strip */}
      <section className="bg-brand-darkRed text-brand-whiteRose">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12 py-16 grid grid-cols-2 md:grid-cols-4 gap-8">
          {stats.map((stat, i) => (
            <div key={i} className="text-center md:text-left">
              <p className="text-4xl md:text-5xl font-heading font-bold text-brand-lightRed mb-1">{stat.number}</p>
      ...
      <section className="max-w-[1400px] mx-auto px-6 lg:px-12 py-24">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-brand-lightRed font-semibold mb-3">What We Do</p>
            <h2 className="text-4xl md:text-5xl font-heading font-bold">Core Capabilities</h2>
          </div>
          <Link to="/services" className="text-sm uppercase tracking-wider font-semibold text-brand-darkRed/60 hover:text-brand-darkRed flex items-center gap-2 transition">
            All Services <ArrowRight size={14} />
          </Link>
        </div>

        <div className="space-y-0 divide-y divide-brand-darkRed/10">
          {services.map((s, i) => (
            <div key={i} className="group py-10 flex flex-col md:flex-row gap-6 md:gap-16 items-start hover:bg-white/50 px-6 -mx-6 transition-colors cursor-pointer">
              <span className="text-brand-lightRed/40 font-heading text-5xl font-bold shrink-0">{s.num}</span>
              <div className="flex-1">
                <h3 className="text-2xl md:text-3xl font-heading font-bold mb-3 group-hover:text-brand-lightRed transition">{s.title}</h3>
                <p className="text-brand-darkRed/60 max-w-lg leading-relaxed">{s.desc}</p>
              </div>
              <ArrowUpRight size={24} className="text-brand-darkRed/20 group-hover:text-brand-darkRed transition shrink-0 mt-2" />
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-white py-24 px-6 lg:px-12">
        <div className="max-w-[1400px] mx-auto">
          <div className="text-center mb-20">
            <p className="text-xs uppercase tracking-[0.3em] text-brand-lightRed font-semibold mb-3">Our Process</p>
            <h2 className="text-4xl md:text-5xl font-heading font-bold">How We Bring Brands to Life</h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0">
            {process.map((item, i) => (
              <div key={i} className="relative p-8 border-l-2 border-brand-lightRed/20 lg:border-l-0 lg:border-t-2 lg:pt-10">
                <div className="absolute top-0 left-0 lg:top-[-9px] lg:left-8 w-4 h-4 bg-brand-lightRed rounded-full -translate-x-[9px] lg:translate-x-0"></div>
                <span className="text-xs uppercase tracking-[0.3em] text-brand-lightRed font-semibold">Step {item.step}</span>
      ...
      <section className="max-w-[1400px] mx-auto px-6 lg:px-12 py-24">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-brand-lightRed font-semibold mb-3">Portfolio</p>
            <h2 className="text-4xl md:text-5xl font-heading font-bold">Featured Work</h2>
          </div>
          <Link to="/product" className="text-sm uppercase tracking-wider font-semibold text-brand-darkRed/60 hover:text-brand-darkRed flex items-center gap-2 transition">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {portfolio.map((item, i) => (
            <div key={i} className={`group relative overflow-hidden cursor-pointer bg-brand-darkRed/5 ${
              item.size === 'lg' ? 'md:row-span-2 min-h-[300px] md:min-h-[500px]' :
              item.size === 'md' ? 'min-h-[250px]' : 'min-h-[250px]'
            }`}>
              <div className="absolute inset-0 bg-brand-darkRed/0 group-hover:bg-brand-darkRed/80 transition-all duration-500 flex items-end">
                <div className="p-8 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                  <p className="text-brand-lightRed text-xs uppercase tracking-widest mb-2">{item.category}</p>
              ...
              <div className="absolute bottom-6 left-8 group-hover:opacity-0 transition-opacity">
                <p className="text-brand-darkRed/30 text-xs uppercase tracking-widest mb-1">{item.category}</p>
                <h3 className="text-brand-darkRed/50 font-heading font-bold text-lg">{item.title}</h3>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="bg-brand-darkRed text-brand-whiteRose py-24 px-6 lg:px-12">
        <div className="max-w-[1400px] mx-auto">
          <p className="text-xs uppercase tracking-[0.3em] text-brand-lightRed font-semibold mb-12 text-center">What Clients Say</p>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            {testimonials.map((t, i) => (
              <div key={i} className="border-t border-white/10 pt-8">
                <div className="flex gap-1 mb-6">
                  {[...Array(5)].map((_, si) => <Star key={si} size={14} className="text-brand-lightRed fill-brand-lightRed" />)}
                </div>
      ...
      <section className="border-b border-brand-darkRed/10 py-20 px-6 lg:px-12">
        <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="max-w-md">
            <h2 className="text-3xl font-heading font-bold mb-2">Stay Inspired</h2>
            <p className="text-brand-darkRed/60 text-sm">Weekly design tips, branding insights, and social media strategies.</p>
          </div>
          <div className="flex gap-0 w-full md:w-auto">
            <input type="email" placeholder="you@company.com" className="flex-1 md:w-72 px-5 py-4 border-2 border-brand-darkRed/10 border-r-0 focus:outline-none focus:border-brand-lightRed bg-white" />
            <button className="bg-brand-darkRed text-white px-6 py-4 font-semibold text-sm uppercase tracking-wider hover:bg-brand-darkRed/90 transition shrink-0">Subscribe</button>
          </div>
        </div>
      </section>
    </div>
